...
def snake_energy(flattened_pts, edge_dist, alpha, beta, gamma):
    """
    ...
    point_distances = np.sqrt(displacements[1:,0]**2 + displacements[1:,1]**2)  ## remove distance of first and last point
    ...
    curvature = (curvature_1d[1:,0]**2 + curvature_1d[1:,1]**2)      ## remove distance of first and last point
    curvature_energy = np.sum(curvature)
    
    # print("Energies")
    # print(external_energy, gamma*external_energy)
    # print(spacing_energy, alpha*spacing_energy)
    # print(curvature_energy, beta*curvature_energy)
    return gamma*external_energy + alpha*spacing_energy + beta*curvature_energy

    
def fit_snake(pts, edge_dist, alpha=0.5, beta=0.25, gamma=5, nits=100, point_plot=None):
    """
    ...
    # optimize
    cost_function = partial(snake_energy, alpha=alpha, beta=beta, gamma=gamma, edge_dist=edge_dist)
    options = {'disp':False}
